...
class Bot_API:
    def __init__(self) -> None:
        ...
        self.fight = 0
        self.arr_dviz = {'1': [640, 1], '2': [1279, 1], '3': [1279, 360], '4': [
            1279, 719], '5': [640, 719], '6': [1, 719], '7': [1, 360], '8': [1, 1]}
        self.move_position = 1
        self.dviz = self.arr_dviz[str(self.move_position)]
        self.fight_one = 0
        self.looting_one = 0
    ...
    def skaning(self):
        ...
        sleep(0.01)
        results = model.predict(path_screen, show=False, save=False, imgsz=(
            1280, 736), conf=config['cnn'])
        mobs = [[]]
        for r in results:
            for c in r.boxes:
        ...
        if int(similarity) <= atack_similarity:
            if self.fight_one == 0:
                ...
                if datetime.now() - self.time_atack >= timedelta(0, 20):
                    self.move_position = self.move_position + \
                        4 if 0 < self.move_position <= 4 else self.move_position-4
                    self.dviz = self.arr_dviz[str(self.move_position)]
